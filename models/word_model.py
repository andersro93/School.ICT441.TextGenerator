...
class WordModel(Model):
    ...
    __unique_words: set = None
    """ A set off all the unique words """
    ...
    def prepare_model(self, print_info=False):
        ...
        self.__unique_words = sorted(list(set(self._raw_content.split())))
        self.__word_to_int_dictionary = dict((word, index) for index, word in enumerate(self.__unique_words))
        ...
        if print_info:
            ...
            print(f"Words: {len(self.__unique_words)}")
    ...
    def generate_text_of_length(self, length: int, verbose=False) -> str:
        ...
        if verbose:
            ...
            print(''.join(self.__get_word_from_number(value) for value in pattern))
        ...
        for i in range(length):
            ...
            # Obtain the predicted word
            predicted_int_for_word = numpy.random.choice(len(predictions[0]), p=predictions[0])

            # Append the translated letter to the return string
            return_string += self.__get_word_from_number(predicted_int_for_word)

            # Add the letter to the next prediction
            pattern = numpy.append(pattern, predicted_int_for_word)
    ...
    def __get_word_from_number(self, number: numpy.ndarray) -> str:
        """
        Helper method for finding the letter for a corresponding number
        :param number:
        :return: str:
        """
        for word in self.__word_to_int_dictionary:
            if self.__word_to_int_dictionary[word] == number:
                return word

        return f'Could not find word for number: {number}'
    ...
    def _format_data_to_x_and_y(self) -> tuple:
        ...
        for i in range(0, len(self._raw_content) - self.pattern_length, 1):
            ...
            data_x.append([self.__word_to_int_dictionary[word] for word in seq_x])
